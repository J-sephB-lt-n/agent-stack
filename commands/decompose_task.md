...
2. Ask me whether I want to decompose this task into sequential units of work (later pieces depend on prior pieces to be finished) or parallel units of work (pieces can be developed in parallel).
3. Ask me clarifying questions if there is anything about your task that is ambiguous or conflicting.
4. Discuss with me the right number of units of work (features) that we want to divide the work into. Provide a recommendation. User stories is a possible approach we might consider. It is important that each unit of work results in a new testable piece of application.
5. Once we've settled on our final partitioning, suggest to me that we write our agreed breakdown of work to `.current_agent_context/features_list.json`, with the following format:
